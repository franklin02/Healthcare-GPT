...
def test_get_page_fetches_with_project_headers():
	...
	with patch("src.scrapers.shared_utils.requests.get", return_value=mock_response) as mock_get:
...
def test_vuln_output_writes_expected_csv_row(tmp_path, monkeypatch):
	...
	rows = list(csv.reader((vulnerabilities_dir / "Example Site.csv").read_text(encoding="utf-8").splitlines()))
	assert rows == [[
		"2026-05-19",
		"2026-05-18",
		"Example Site",
...
def test_noise_output_writes_expected_csv_row(tmp_path, monkeypatch):
	...
	with patch("src.scrapers.shared_utils.datetime.datetime") as mock_datetime:
		...
		shared_utils.noise_output("Example Site", "Ignored article", "https://example.com", "body\ntext", "irrelevant")

	rows = list(csv.reader((noise_dir / "Example Site.csv").read_text(encoding="utf-8").splitlines()))
	assert rows == [[
		"2026-05-19 10:30",
		"Example Site",
		"Ignored article",
...
def test_build_page_url_uses_base_url_for_starting_page():
	...
	assert shared_utils.build_page_url({"url": "https://example.com", "map": {}}, 1, 1, "page") == "https://example.com"


def test_build_page_url_uses_mapped_page_param():
	"""Test that build_page_url uses the configured page param when present."""
	assert shared_utils.build_page_url(
...
def test_build_page_url_falls_back_to_default_page_param():
	...
	assert shared_utils.build_page_url({"url": "https://example.com/articles", "map": {}}, 2, 1, "page") == "https://example.com/articles?page=2"
